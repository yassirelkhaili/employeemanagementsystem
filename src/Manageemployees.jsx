...
const Manageemployees = () => {
    ...
    const handleSubmit = async () => {
        try {
            await axios.post("http://localhost:8000/", input)
            console.log(input);
        } catch (err) {
            console.log(err);
        }
    ...
    const handleDelete = async (id) => {
        await axios.delete("http://localhost:8000/"+id).then(
            res => console.log(res)
            ).catch((err) => {
                console.log(err);
            });
    ...
    const handlesave = async (e) => {
        await axios.put("http://localhost:8000/"+id, input).then(
            res => console.log(res)
        ).catch((err) => {
            console.log(err);
        })
  ...
  return (
    ...
    <div className='container-fluid bg-dark' ref={addform}>
            ...
            <button className='btn btn-primary' id='add' onClick={handleSubmit}>Add Employee</button>
            </div>
        </form>
    </div>
    <div className='container'>
          <table className='table table-bordered' ref={employeedata}>
            ...
            <tbody>
                {employees.map((employee) => {
                    return (
                        <tr key={employee.id}>
                            <th scope='row' id='id'>{employee.id}</th>
                            ...
                            <td id='action' className='actions'>
                                ...
                                <button className='btn btn-danger' onClick={() => {handleDelete(employee.id); console.log(employee.id)}}>Delete</button>
                            </td>
                        </tr>
                    )
                })}
          ...
          <div className='github'><a href="https://github.com/yassirelkhaili" className='link-info' target="_blank" rel="noreferrer">Source Code on Github</a></div>
    </div>
    </>
  )
}
